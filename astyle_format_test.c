...
void long_function_name(int param1, // 错误：参数应该缩进
                        int param2);
...
// --pad-comma (逗号填充)
void comma_example(int a, int b, int c)
{
} // 错误：逗号后面应该加空格

// --pad-first-paren-out (首括号外填充)
void first_paren_example(int x)
{
} // 错误：函数名后不应该有空格
...
void param_type_example(const int *x)
{
} // 错误：参数类型后应该加空格

// --unpad-paren (去除括号内多余空格)
void unpad_example(int x, int y)
{
} // 错误：括号内不应该有额外空格
...
void very_long_function_name_with_many_characters_that_exceeds_the_maximum_line_length_of_120_characters(int parameter1,
                                                                                                         int parameter2)
{
    // 这个函数名超过120字符
}
...
{
    int x = very_long_function_name(parameter1, parameter2,
...
void one_line_if()
{
    if (1) return; // 错误：if语句应该换行
}

// --add-brackets (添加括号)
void no_brackets()
{
    if (1) return;                                  // 错误：应该添加括号
...
// 函数声明
void function_declaration(int param); // 错误：括号位置不正确
